...
from Networks import SimpleGAN
...
gan = SimpleGAN(img_width=image_width, img_height=image_height, latent_size=128)
gan.build_models()
d_acc = []
g_loss = []
...
for epoch in range(0, EPOCH_NUM):
    for step in range(0, steps_per_epoch):
        ...
        print('Epoch {0}: Batch {1}/{2} - Generator Loss: {3:3,.3f} - Discriminator Acc.: {4:3,.3f}'.format(
            gan.adversarial_model.n_epochs + 1, step, steps_per_epoch, cum_g_loss, cum_d_acc))
        if step % 1000 == 0 or step == (steps_per_epoch - 1):
            generate_images(gan.generator,
                            'learning_progress/{}/epoch{}_{}.png'.format(PATH, gan.adversarial_model.n_epochs, step))
    gan.adversarial_model.n_epochs += 1
    gan.reset_metrics()

    d_acc.append(cum_d_acc)
    ax = sns.lineplot(range(gan.adversarial_model.n_epochs), d_acc)
    ...
    ax = sns.lineplot(range(gan.adversarial_model.n_epochs), g_loss)
