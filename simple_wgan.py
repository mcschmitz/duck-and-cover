...
for epoch in range(0, EPOCH_NUM):
    for step in range(0, steps_per_epoch):
        ...
        print('Epoch {0}: Batch {1}/{2} - Generator Loss: {3:3,.3f} - Discriminator Loss + : {4:3,.3f} '
            ...
            wgan.combined_model.n_epochs + 1, step, steps_per_epoch, cum_g_loss, cum_d_acc[0], cum_d_acc[1],
            cum_d_acc[2]))

        if step % 1000 == 0 or step == (steps_per_epoch - 1):
            generate_images(wgan.generator,
                            'learning_progress/{}/epoch{}_{}.png'.format(PATH, wgan.combined_model.n_epochs, step))
    wgan.combined_model.n_epochs += 1
    wgan.reset_metrics()

    d_loss_positives.append(cum_d_acc[0])
    ax = sns.lineplot(range(wgan.combined_model.n_epochs), d_loss_positives)
    ...
    ax = sns.lineplot(range(wgan.combined_model.n_epochs), g_loss)
